...
-- ==========================================
-- EXECUTOR SAFETY POLYFILLS
-- Prevents 'nil value' crashes if functions are missing
-- ==========================================
local cloneref = cloneref or function(o) return o end
local gethui = gethui or function() return CoreGui end
local request = (syn and syn.request) or (http and http.request) or http_request or (fluxus and fluxus.request) or request or function() return {Body = nil} end
...
local Blacklist = {}
if not Config.BlacklistedWords then Config.BlacklistedWords = {} end
for _, w in ipairs(Config.BlacklistedWords) do Blacklist[w] = true end

-- NEW: Setup Whitelist Memory
local Whitelist = {}
if not Config.WhitelistedWords then Config.WhitelistedWords = {} end
for _, w in ipairs(Config.WhitelistedWords) do Whitelist[w] = true end
...
local function FetchWords()
    ...
    if success and res and res.Body then
        if writefile then -- NEW: Safety check added here
...
lhotshampa = true,
lhiamba = true,
sphenopalatineganglioneuralgia = true,
keychain = true,
keychains = true,
eep = true,
eeps = true,
...
eeproms = true,
roblox = true,
yangchuanosauruses = true,
allylisopropylacetamides = true,
nzi = true,
nzara = true,
nzima = true,
nzem = true,
carboxyhemoglobin = true,
wtv = true,
imao = true,
lmfao = true,
geranylgeranyltransferase = true,
geranylgeranyltransferases = true,
trinitrophenylmethylnitramines = true,
trihexyltetradecylphosphonium = true,
ventriculocisternostomy = true,
esophagogastroduodenoscopies = true,
esophagogastroduodenoscopy = true,
xanthogranulomatous = true,
wowie = true,
picturesque = true,
ndongo = true
}
...
local function LoadList(fname)
    ...
    if isfile and isfile(fname) then -- NEW: Added 'isfile and' safety check
...
local HardLetterScores = {
    ze = 17, zy = 19, yc = 21, nk = 19, bt = 21, bk = 21, pk = 21, hd = 21,
    hl = 20, fd = 21, kt = 21, sz = 20, ez = 17, ky = 11, py = 11, dh = 20, ly = 10,
    x = 10, z = 8, q = 7, j = 7, v = 9, y = 8, k = 9,
    mb = 20, sd = 21, dz = 21, md = 21, os = 11, od = 8, vy = 21,
    lh = 21, sf = 21, pf = 21, dm = 21, wt = 21, fc = 21, tx = 21, hm = 21,
    sb = 21, er = 17, uv = 17, gy = 19, yg = 21, xo = 20, cy = 17, ox = 19, cw = 21,
    lw = 20, yp = 21, jy = 21, nz = 21, dae = 20, aly = 20, pse = 20, epia = 48, ving = 19,
    jji = 20, ers = 20, ndo = 20, nga = 20, adz = 20, tze = 20, tsi = 20, ror = 20, gia = 20,
    nz = 47, eep = 20, ngo = 20
...
local function GetKillerScore(word)
    ...
    return HardLetterScores[word:sub(-4)]
        or HardLetterScores[word:sub(-3)]
        or HardLetterScores[word:sub(-2)]
        or HardLetterScores[word:sub(-1)]
        or 0
end
...
local function GetCurrentGameWord(providedFrame)
    ...
    local rawWord = detected:lower():gsub(" ", "")
    local cleanWord = rawWord:gsub("%.", ""):gsub("…", "")
    
    -- If we detect truncation dots, stitch the new visible text onto our memory
    if rawWord:find("%.") or rawWord:find("…") then
        if #WordMemoryState > 0 then
            local overlapLen = 0
            -- Find the exact overlap between the end of the memory and the start of the new text
            for len = math.min(#cleanWord, #WordMemoryState), 1, -1 do
                if WordMemoryState:sub(-len) == cleanWord:sub(1, len) then
                    overlapLen = len
                    break
                end
            end
            WordMemoryState = WordMemoryState:sub(1, #WordMemoryState - overlapLen) .. cleanWord
        else
            WordMemoryState = cleanWord
        end
    else
                -- No dots detected, which means this is a brand new word. Reset the memory!
        WordMemoryState = cleanWord
    end
    
    return WordMemoryState, censored
end
...
-- ==========================================
-- OPTIMIZED BLACKLIST MANAGER GUI & LOGIC
-- ==========================================

-- Ensure Config exists to prevent looping errors
if not Config.BlacklistedWords then Config.BlacklistedWords = {} end
for _, w in ipairs(Config.BlacklistedWords) do Blacklist[w] = true end

-- ==========================================
-- BUTTON RESIZING (Blacklist & Whitelist)
-- ==========================================
...
ManageBlacklistBtn.TextColor3 = Color3.fromRGB(255, 100, 100)
ManageBlacklistBtn.BackgroundColor3 = THEME.Background
ManageBlacklistBtn.Size = UDim2.new(0, 130, 0, 24) -- Resized to half width
ManageBlacklistBtn.Position = UDim2.new(0, 15, 0, 235) 
Instance.new("UICorner", ManageBlacklistBtn).CornerRadius = UDim.new(0, 4)

local ManageWhitelistBtn = Instance.new("TextButton", TogglesFrame)
ManageWhitelistBtn.Text = "Manage Whitelist"
ManageWhitelistBtn.Font = Enum.Font.GothamMedium
ManageWhitelistBtn.TextSize = 11
ManageWhitelistBtn.TextColor3 = THEME.Success
ManageWhitelistBtn.BackgroundColor3 = THEME.Background
ManageWhitelistBtn.Size = UDim2.new(0, 130, 0, 24) -- Placed next to Blacklist
ManageWhitelistBtn.Position = UDim2.new(0, 150, 0, 235) 
Instance.new("UICorner", ManageWhitelistBtn).CornerRadius = UDim.new(0, 4)

-- ==========================================
-- WHITELIST MANAGER GUI
-- ==========================================
local WhitelistFrame = Instance.new("Frame", ScreenGui)
WhitelistFrame.Name = "WhitelistFrame"
WhitelistFrame.Size = UDim2.new(0, 250, 0, 350)
WhitelistFrame.Position = UDim2.new(0.5, -385, 0.5, -175)
WhitelistFrame.BackgroundColor3 = THEME.Background
WhitelistFrame.Visible = false
WhitelistFrame.ClipsDescendants = true
EnableDragging(WhitelistFrame)
Instance.new("UICorner", WhitelistFrame).CornerRadius = UDim.new(0, 8)
local WLStroke = Instance.new("UIStroke", WhitelistFrame)
WLStroke.Color = THEME.Success
WLStroke.Transparency = 0.5
WLStroke.Thickness = 2

local WLHeader = Instance.new("TextLabel", WhitelistFrame)
WLHeader.Text = "Whitelist Manager"
WLHeader.Font = Enum.Font.GothamBold
WLHeader.TextSize = 14
WLHeader.TextColor3 = THEME.Text
WLHeader.Size = UDim2.new(1, 0, 0, 35)
WLHeader.BackgroundTransparency = 1

local WLCloseBtn = Instance.new("TextButton", WhitelistFrame)
WLCloseBtn.Text = "X"
WLCloseBtn.Font = Enum.Font.GothamBold
WLCloseBtn.TextSize = 14
WLCloseBtn.TextColor3 = THEME.Success
WLCloseBtn.Size = UDim2.new(0, 30, 0, 30)
WLCloseBtn.Position = UDim2.new(1, -30, 0, 2)
WLCloseBtn.BackgroundTransparency = 1
WLCloseBtn.MouseButton1Click:Connect(function() WhitelistFrame.Visible = false end)

ManageWhitelistBtn.MouseButton1Click:Connect(function()
    WhitelistFrame.Visible = not WhitelistFrame.Visible
    WhitelistFrame.Parent = nil
    WhitelistFrame.Parent = ScreenGui
end)

local WLSearchBox = Instance.new("TextBox", WhitelistFrame)
WLSearchBox.Font = Enum.Font.Gotham
WLSearchBox.TextSize = 12
WLSearchBox.BackgroundColor3 = THEME.ItemBG
WLSearchBox.Size = UDim2.new(1, -20, 0, 24)
WLSearchBox.Position = UDim2.new(0, 10, 0, 35)
Instance.new("UICorner", WLSearchBox).CornerRadius = UDim.new(0, 4)
SetupPhantomBox(WLSearchBox, "Search whitelist...")

local WLScroll = Instance.new("ScrollingFrame", WhitelistFrame)
WLScroll.Size = UDim2.new(1, -10, 1, -110)
WLScroll.Position = UDim2.new(0, 5, 0, 65)
WLScroll.BackgroundTransparency = 1
WLScroll.ScrollBarThickness = 2
WLScroll.ScrollBarImageColor3 = THEME.Success
WLScroll.CanvasSize = UDim2.new(0,0,0,0)

local WLListLayout = Instance.new("UIListLayout", WLScroll)
WLListLayout.SortOrder = Enum.SortOrder.LayoutOrder
WLListLayout.Padding = UDim.new(0, 2)

local WLCommandBox = Instance.new("TextBox", WhitelistFrame)
WLCommandBox.Font = Enum.Font.Gotham
WLCommandBox.TextSize = 12
WLCommandBox.BackgroundColor3 = THEME.ItemBG
WLCommandBox.Size = UDim2.new(0, 170, 0, 24)
WLCommandBox.Position = UDim2.new(0, 10, 1, -35)
Instance.new("UICorner", WLCommandBox).CornerRadius = UDim.new(0, 4)
SetupPhantomBox(WLCommandBox, "add / remove")

local WLExecuteBtn = Instance.new("TextButton", WhitelistFrame)
WLExecuteBtn.Text = "Exe"
WLExecuteBtn.Font = Enum.Font.GothamBold
WLExecuteBtn.TextSize = 11
WLExecuteBtn.TextColor3 = THEME.Success
WLExecuteBtn.BackgroundColor3 = THEME.ItemBG
WLExecuteBtn.Size = UDim2.new(0, 50, 0, 24)
WLExecuteBtn.Position = UDim2.new(1, -60, 1, -35)
Instance.new("UICorner", WLExecuteBtn).CornerRadius = UDim.new(0, 4)
...
local function RefreshWhitelist()
    ...
    local queryRaw = WLSearchBox.Text
    local query = (queryRaw == "Search whitelist...") and "" or string.lower(string.gsub(queryRaw, "[%s%c]+", ""))
    local list = Config.WhitelistedWords or {}
    local shownCount = 0
    
    for _, w in ipairs(list) do
        if query == "" or string.find(w, query, 1, true) then
            shownCount = shownCount + 1
            ...
            del.MouseButton1Click:Connect(function()
                for idx, savedWord in ipairs(Config.WhitelistedWords) do
                    if savedWord == w then table.remove(Config.WhitelistedWords, idx) break end
                end
                Whitelist[w] = nil
                SaveConfig()
                RefreshWhitelist()
                ShowToast("Un-whitelisted: " .. w, "warning")
            end)
        end
    end
    WLScroll.CanvasSize = UDim2.new(0, 0, 0, shownCount * 24)
end

WLSearchBox:GetPropertyChangedSignal("Text"):Connect(RefreshWhitelist)

WLExecuteBtn.MouseButton1Click:Connect(function()
    local text = WLCommandBox.Text
    if text == "add / remove" or text == "" then return end
    
    local args = {}
    for word in string.gmatch(text, "%S+") do table.insert(args, string.lower(word)) end
    
    local command = args[1] or ""
    local word1 = args[2] and string.gsub(args[2], "[%s%c]+", "") or ""

    if command == "add" then
        if word1 == "" then ShowToast("Specify a word to whitelist!", "error") return end
        
        -- Also add to dictionary automatically if it's missing
        local inDict = false
        for _, dictWord in ipairs(Words) do
            if dictWord == word1 then inDict = true break end
        end
        if not inDict then
            table.insert(Words, word1)
            local c = word1:sub(1,1)
            if c == "" then c = "#" end
            Buckets[c] = Buckets[c] or {}
            table.insert(Buckets[c], word1)
        end
        
        local exists = false
        for _, w in ipairs(Config.WhitelistedWords) do
            if w == word1 then exists = true break end
        end
        
        if exists then
            ShowToast("Word is already whitelisted!", "warning")
        else
            table.insert(Config.WhitelistedWords, word1)
            Whitelist[word1] = true
            ShowToast("Whitelisted: " .. word1, "success")
        end
        
    elseif command == "remove" then
        if word1 == "" then ShowToast("Specify a word to remove!", "error") return end
        local found = false
        for i, w in ipairs(Config.WhitelistedWords) do
            if w == word1 then
                table.remove(Config.WhitelistedWords, i)
                Whitelist[w] = nil
                found = true
                ShowToast("Removed from whitelist: " .. w, "warning")
                break
            end
        end
        if not found then ShowToast("Word not found in whitelist!", "error") end
    else
        ShowToast("Use: add [word] or remove [word]", "error")
    end

    SaveConfig()
    WLCommandBox.Text = ""
    WLCommandBox:ReleaseFocus()
    RefreshWhitelist()
    if UpdateList then forceUpdateList = true end -- Force refresh the words list UI
end)
...
BLScroll.Size = UDim2.new(1, -10, 1, -110)
...
local BLCommandBox = Instance.new("TextBox", BlacklistFrame)
BLCommandBox.Font = Enum.Font.Gotham
BLCommandBox.TextSize = 12
BLCommandBox.BackgroundColor3 = THEME.ItemBG
BLCommandBox.Size = UDim2.new(0, 170, 0, 24)
BLCommandBox.Position = UDim2.new(0, 10, 1, -35)
Instance.new("UICorner", BLCommandBox).CornerRadius = UDim.new(0, 4)
SetupPhantomBox(BLCommandBox, "add / remove / replace")

local BLExecuteBtn = Instance.new("TextButton", BlacklistFrame)
BLExecuteBtn.Text = "Exe"
BLExecuteBtn.Font = Enum.Font.GothamBold
BLExecuteBtn.TextSize = 11
BLExecuteBtn.TextColor3 = Color3.fromRGB(255, 100, 100)
BLExecuteBtn.BackgroundColor3 = THEME.ItemBG
BLExecuteBtn.Size = UDim2.new(0, 50, 0, 24)
BLExecuteBtn.Position = UDim2.new(1, -60, 1, -35)
Instance.new("UICorner", BLExecuteBtn).CornerRadius = UDim.new(0, 4)
...
local function RefreshBlacklist()
    ...
    local query = (queryRaw == "Search blacklist...") and "" or string.lower(string.gsub(queryRaw, "[%s%c]+", ""))
    
    local list = Config.BlacklistedWords or {}
    local shownCount = 0
    ...
    for _, w in ipairs(list) do
        if query == "" or string.find(w, query, 1, true) then
            ...
            del.MouseButton1Click:Connect(function()
                -- Safe removal logic to prevent list corruption
                for idx, savedWord in ipairs(Config.BlacklistedWords) do
                    if savedWord == w then
                        table.remove(Config.BlacklistedWords, idx)
                        break
                    end
                end
                Blacklist[w] = nil
...
BLExecuteBtn.MouseButton1Click:Connect(function()
    local text = BLCommandBox.Text
    if text == "add / remove / replace" or text == "" then return end
    
    -- Universal safe string parsing (Works on all executors)
    local args = {}
    ...
    local command = args[1] or ""
    local word1 = args[2] and string.gsub(args[2], "[%s%c]+", "") or ""
    local word2 = args[3] and string.gsub(args[3], "[%s%c]+", "") or ""

    if command == "add" then
        if word1 == "" then ShowToast("Specify a word to blacklist!", "error") return end
        if #word1 > 35 then ShowToast("Word is too long and was ignored!", "error") return end
        
        local exists = false
        for _, w in ipairs(Config.BlacklistedWords) do
            if w == word1 then exists = true break end
        end
        ...
        if exists then
            ...
            table.insert(Config.BlacklistedWords, word1)
            Blacklist[word1] = true
            ShowToast("Blacklisted: " .. word1, "success")
        end
    ...
    elseif command == "remove" then
        ...
        local found = false
        for i, w in ipairs(Config.BlacklistedWords) do
            if w == word1 then
                table.remove(Config.BlacklistedWords, i)
                Blacklist[w] = nil
                found = true
                ShowToast("Removed from blacklist: " .. w, "warning")
                break
            end
        end
        if not found then ShowToast("Word not found in blacklist!", "error") end
        
    elseif command == "replace" then
        if word1 == "" or word2 == "" then ShowToast("Usage: replace [old] [new]", "error") return end
        if #word2 > 35 then ShowToast("New word is too long and was ignored!", "error") return end
        
        local found = false
        for i, w in ipairs(Config.BlacklistedWords) do
            if w == word1 then
                Config.BlacklistedWords[i] = word2
                Blacklist[word1] = nil
                Blacklist[word2] = true
                found = true
                ShowToast("Replaced " .. word1 .. " with " .. word2, "success")
                break
            end
        end
        if not found then ShowToast("Word not found to replace!", "error") end
    else
        ShowToast("Use: add [word], remove [word], or replace [old] [new]", "error")
    end

    SaveConfig()
    BLCommandBox.Text = ""
    BLCommandBox:ReleaseFocus()
    RefreshBlacklist()
end)
    ...
    local success, err = pcall(function()
        if isCorrection then
            ...
            if not riskyMistakes then
                ...
                if finalCheck then
                    -- Fix for the "..." truncation bug on long words
                    local checkText = finalCheck.Text:lower():gsub(" ", ""):gsub("%.", ""):gsub("…", "")
                    if checkText ~= "" and targetWord:sub(-#checkText) ~= checkText then
            ...
            while (tick() - verifyStart) < 1.5 do
                ...
                -- Check if the current word matches the end of the target word (accounts for truncation)
                local isMatch = (currentCheck == targetWord) or (currentCheck ~= "" and targetWord:sub(-#currentCheck) == currentCheck)
                if currentCheck == "" or (not isMatch and currentCheck ~= currentDetected) then
                     accepted = true
                     break
                end
                task.wait(0.05)
            ...
            if not accepted then
                ...
                if postCheck then
                     local checkText = postCheck.Text:lower():gsub(" ", ""):gsub("%.", ""):gsub("…", "")
                     if checkText ~= "" and targetWord:sub(-#checkText) == checkText then
    ...
    local function CollectMatches(prefix, tryFallbackLengths)
        ...
        if bucket then
            local checkWord = function(w)
                ...
                -- Check for Whitelist safety
                local isWhitelisted = Whitelist and Whitelist[w]
                
                -- If it is NOT whitelisted, apply normal filters
                ...
                if mLen == #prefix then
                    if isWhitelisted then
                        table.insert(exacts, 1, w) -- Force Whitelisted to top
                    else
            ...
            if useBinary and #prefix > 0 then
                ...
                local searchLimit = (sortMode == "Random") and 1000 or math.huge
                if bucket == Words and searchLimit == math.huge then searchLimit = 5000 end
...
if not success then
    warn("REAL SCRIPT ERROR: " .. tostring(err))
    print("REAL SCRIPT ERROR: " .. tostring(err))
end
